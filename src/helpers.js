    ...
    if (!listadoCursos.length) {
        ...
        listadoCursos.forEach(curso => {
            ...
            curso.matriculados.forEach(usuario => {
                ...
                texto = texto + `<tr id="idcursoestudiante">
          ...
          if (!listado) {
            ...
            if (!disponibles) {
              ...
              let texto = `<form action='/matricula' method='post'>
                            <div class='form-row'>
                              <div class='form-group col-md-2'>
                                <select class='form-control' style='width:200px' name='idcurso' id='idcurso' required>";
                                  <option selected></option>`;
              disponibles.forEach(curso => {
              ...
              texto = texto + `</select>
                              ...
                              <div class="form-group col-md-6">
                                <button type="submit" class="btn btn-dark">Matricular curso</button>
                              </div>
                            </div>
                          </form>`;
              return texto;
...
hbs.registerHelper('miscursos', (cursos, listado) => {
    if (!cursos.length) {
...
hbs.registerHelper('integrantesCurso', (curso, listadoUsuarios) => {
  if (!curso) {
    return "El id del curso que ingresaste no existe en la base de datos, intentalo de nuevo!";
  }else {
    let texto = `<h3>Lista integrantes del curso ${curso.nombre}</h3>
                <br>
                  <table class='table table-striped'>
                    <thead class='thead-dark'>
                      <th> Nombre </th>
  ...
  if (!listadoU) {
    return `<form action="/registro" method="get">
      ...
      let texto = `<form action='/actualizarUsuario' method='post'>
                    <div class='form-row'>
                      <div class='form-group col-md-4'>
                        <select class='form-control' style='width:500px' name='documento' id='documento' required>";
...
hbs.registerHelper('mostrarRol',(perfil)=>{
  if (!perfil) {
    return ""
  }else {
    let texto;
    texto=texto + `<div class="d-flex justify-content-center align-items-center container">
                    <div class="row">
                      <form class="well form-horizontal" action="/cambioRol" method="post">
                        <h3 style="text-align:center">Cambio del rol</h3>
                          <div class="form-row">
                            <div class="form-group col-md-6">
                            ...
                            <div class="form-group col-md-6">
                            <button class="btn btn-dark" value="${perfil.documento}" name="documento">cambiar rol</button>
                            </div>
                          </div>
                          <div class="form-row">
                            <div class="form-group col-md-12">
                              ...
                              <br>
                              <h3 style="text-align:center; color:red;"><b>ADVERTENCIA:</b> Al hacer click en cambiar rol, se eliminaran <b>TODOS</b> los cursos inscritos!</h3>
                            </div>
                          </div>
                        </form>
                      </div>
...
hbs.registerHelper('listarCursosDocente', (listadoCursos, listadoUsuarios, cursosAsignados) => {
    let  i = 1;
    let texto;
    if (!cursosAsignados.length) {
        return "No tienes cursos asignados";
    } else {
        cursosAsignados.forEach(id => {
          let curso=listadoCursos.find(cur => cur.id === id);
            texto = texto + `<div class="panel-group">
                              <div class="panel panel-default">
                                <div class="panel-heading">
                                  <h4 class="panel-title">
                                  <a data-toggle="collapse" href="#collapse${i}"> Curso: ${curso.nombre} -- Id: ${curso.id} -- Estado: ${curso.estado} -- Matriculados: (${curso.matriculados.length}) -- Intensidad horaria: ${curso.ih} -- Modalidad: ${curso.modalidad}</a>
                                  </h4>
                                </div>
                                <div id="collapse${i}" class="panel-collapse collapse">`;
            texto = texto + `<div class="panel-body">
